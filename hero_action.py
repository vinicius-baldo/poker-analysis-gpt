...
class HeroAction:
    ...
    def execute_action(self, button_coordinates, action, amount):
        ...
        match action:
            ...
            case _:
                print(
                    f"{Fore.RED}HeroActions -> execute_action(): Player Action {action} not recognized."
                )
    ...
    def click_button(self, button_coordinates):
        ...
        if not self.window:
            print(
                f"{Fore.RED}HeroActions -> click_button(): Window not located. Please locate the window first."
            )
            return
        ...
        abs_x = self.window.left + int(
            self.window.width * (button_x + random_position_x)
        )
        abs_y = self.window.top + int(
            self.window.height * (button_y + random_position_y)
        )
        ...
        # Move the mouse cursor to the position for visual confirmation
        pyautogui.moveTo(
            (self.window.width / 2) + random.uniform(0, 500),
            (self.window.height / 2) + random.uniform(0, 500),
    ...
    def click_max_bet_button(self):
        ...
        if not self.window:
            print(
                f"{Fore.RED}HeroActions -> Window not located. Please locate the window first."
            )
            return
        ...
        abs_x = self.window.left + int(
            self.window.width * (0.945 + random_position_add)
        )
        abs_y = self.window.top + int(
            self.window.height * (0.809 + random_position_add)
        )
        ...
        # Move the mouse cursor to the position for visual confirmation
        pyautogui.moveTo(
            abs_x, abs_y, duration=random_cursor_speed
        )  # Move the cursor over 1 second

        # Perform the click
        pyautogui.click(abs_x, abs_y)

        print(
            f"{Fore.YELLOW}HeroActions -> Max bet Button clicked at x={abs_x}, y={abs_y}"
        )
    ...
    def click_and_drag_input_box(
        self, start_rel_x, start_rel_y, end_rel_x, end_rel_y, duration=0.1
    ):
        """
        ...
        )  # Short delay to ensure the mouse down event is registered
    ...
    def activate_window(self):
        ...
        if self.window:
            ...
            if not self.window_activation_error_reported:
                print(
                    f"{Fore.RED}HeroActions -> Window not located or cannot be activated."
                )
                self.window_activation_error_reported = True
