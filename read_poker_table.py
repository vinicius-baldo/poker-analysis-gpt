...
class ReadPokerTable:
    ...
    def __init__(
        ...
        self.last_action_player = (
            0  # For detecting player actions and stack sizes only once
        )

        self.hero_buttons_active = {}  # Detected active hero buttons

        self.action_processed = (
            False  # Detect heros action only once when action buttons become active
        )

        self.last_detected_cards = (
            []
        )  # Add a local variable to keep track of the last detected cards
    ...
    def activate_window(self):
        ...
        if self.window:
            try:
                ...
                self.window_activation_error_reported = (
                    False  # Reset the flag if activation is successful
                )
            ...
            if self.contains_white(screenshot):
                ...
                if button_text:
                    ...
                    if "Raise" in cleaned_button_text:
                        ...
                        keyword in cleaned_button_text
                        for keyword in ["Fold", "Call", "Check", "Resume", "Cash"]
                    ):
                        any_button_active = True
                        self.hero_buttons_active[i] = {
                            "action": cleaned_button_text,
                            "pos": (x, y),
                        }

                    time.sleep(
                        0.4
                    )  # This delay can be adjusted or removed based on requirements
            ...
            if not self.action_processed:
                ...
                if self.game_state.round_count > 0:
                    ...
                    if self.game_state.current_board_stage == "Pre-Flop":
                        ...
                        hero_role = self.game_state.players[
                            self.game_state.hero_player_number
                        ].get("role")
                        hero_cards = self.game_state.players[
                            self.game_state.hero_player_number
                        ].get("cards")
                        ...
                        if hero_cards:
                            is_playable_card = self.hero_hand_range.is_hand_in_range(
                                hero_cards
                            )
                        ...
                        if is_playable_card:
                            ...
                            analysis_thread = threading.Thread(
                                target=self.analyze_and_log
                            )
                            analysis_thread.start()
                            print(
                                f"{Fore.GREEN} PLAYABLE CARD: {hero_cards} in {hero_role} ROLE"
                            )
                            ...
                            print(
                                f"{Fore.RED} UNPLAYABLE CARD: {hero_cards} in {hero_role} ROLE "
                            )
    ...
    def analyze_and_log(self):
        ...
        action_result = self.poker_assistant.AnalyzeAI(
            self.hero_buttons_active, self.game_state.get_ai_log()
        )
    ...
    def image_hash(self, image):
        ...
        return "".join(
            "1" if x > avg else "0" for x in gray.flatten()
    ...
    def detect_player_stack_and_action(self, player_number):
        ...
        if current_stack_size is not None:
            if (
                stack_size_change < 20
            ):  # Avoid updating wrong values which sometimes might be in the hundreds for detected stack size by OCR
                bet_amount = (
                    stack_size_change  # Convert to positive value for bet amount
                )
    ...
    def is_gray_bar_present(self, x, y):
        try:
            ...
            in_range = all(
                gray_lower[i] <= pixel_color[i] <= gray_upper[i] for i in range(3)
            )
            return in_range
    ...
    def detect_total_pot_size(self):
        ...
        detected_text = self.detect_text(
            0.445, 0.312, 110, 28
        )  # Coordinates for the total pot size
        ...
        try:
            ...
            print(
                f"Unable to parse total pot size from detected text: '{detected_text}'"
            )
    ...
    def find_player_cards(self, player_number):
        ...
        for index, (num_x, num_y) in enumerate(
            player_card_positions[player_number], start=1
        ):
            ...
            # Process card suit
            card_filename = (
                f"card{index}Icon_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            )

            screenshot_icon = self.capture_screen_area(
                icon_x, icon_y, icon_width, icon_height
            )  # , filename=card_filename)
            screenshot_icon_gray = cv2.cvtColor(
                np.array(screenshot_icon), cv2.COLOR_BGR2GRAY
            )

            card_suit = None
            for suit, template in self.card_icon_templates.items():
                result = cv2.matchTemplate(
                    screenshot_icon_gray, template, cv2.TM_CCOEFF_NORMED
                )
            ...
            card2_filename = (
                f"card{index}Letter_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            )

            screenshot_num = self.capture_screen_area(
                num_x, num_y, num_width, num_height
            )  # , filename=card2_filename)
            screenshot_num_gray = cv2.cvtColor(
                np.array(screenshot_num), cv2.COLOR_BGR2GRAY
            )

            card_rank = None
            for rank, template in self.card_number_templates.items():
                result = cv2.matchTemplate(
                    screenshot_num_gray, template, cv2.TM_CCOEFF_NORMED
                )
                _, max_val, _, _ = cv2.minMaxLoc(result)
                if max_val > 0.9:
                    card_rank = rank
    ...
    def find_cards_on_table(self):
        ...
        with self.game_state_lock:
            ...
            if not valid_cards_found:
                ...
                if valid_cards_found != self.last_detected_cards and len(
                    valid_cards_found
                ) >= len(self.last_detected_cards):
                ...
                if self.game_state.dealer_position != player_number:
                    ...
                    with self.game_state_lock:
                        ...
                        if self.game_state.round_count > 1:
                            if (
                                self.game_state.round_count % 12 == 0
                            ):  # Do every 8 rounds
                ...
                if self.check_player_card_active(player_number):
                    ...
                    if current_cards_detected_length == 2:
                        ...
                        with self.game_state_lock:
                            ...
                            current_cards_stored = self.game_state.players.get(
                                player_number, {}
                            ).get("cards")
                            ...
                            if new_cards_detected != current_cards_stored:
                                ...
                                self.game_state.update_player(
                                    player_number, cards=new_cards_detected
                                )
    ...
    def start_continuous_detection(self):
        ...
        action_detection_thread1 = threading.Thread(
            target=self.continuous_detection_player_action1
        )

        action_detection_thread2 = threading.Thread(
            target=self.continuous_detection_player_action2
        )

        action_detection_thread3 = threading.Thread(
            target=self.continuous_detection_player_action3
        )

        action_detection_thread4 = threading.Thread(
            target=self.continuous_detection_player_action4
        )

        action_detection_thread5 = threading.Thread(
            target=self.continuous_detection_player_action5
        )

        action_detection_thread6 = threading.Thread(
            target=self.continuous_detection_player_action6
        )
        ...
        cards_detection_thread = threading.Thread(
            target=self.continuous_detection_table_cards
        )

        player_cards_detection_thread = threading.Thread(
            target=self.continuous_detection_player_cards
        )
