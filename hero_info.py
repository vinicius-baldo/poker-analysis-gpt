...
class HeroInfo:
    ...
    def update_action_count(self, round_number, hero_role, board_stage, hero_action):
        ...
        if board_stage not in self.action_counts[hero_role]:
            self.action_counts[hero_role][board_stage] = {
                "Fold": 0,
                "Check": 0,
                "Call": 0,
                "Raise": 0,
    ...
    def get_recent_actions(self):
        ...
        for round_number in sorted(self.recent_actions.keys())[-10:]:
            ...
            for hero_role, board_stage, hero_action in self.recent_actions[
                round_number
            ]:
    ...
    def total_bets(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get(stage, {}).get("Bet", 0)
    ...
    def total_raises(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get(stage, {}).get("Raise", 0)
            for role in self.action_counts
            for stage in self.action_counts.get(role, {})
        )
    ...
    def total_checks(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get(stage, {}).get("Check", 0)
            for role in self.action_counts
            for stage in self.action_counts.get(role, {})
        )
    ...
    def total_folds(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get(stage, {}).get("Fold", 0)
            for role in self.action_counts
            for stage in self.action_counts.get(role, {})
        )
    ...
    def total_folds_preflop(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Pre-Flop", {}).get("Fold", 0)
            for role in self.action_counts
        )
    ...
    def total_raises_preflop(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Pre-Flop", {}).get("Raise", 0)
            for role in self.action_counts
        )
    ...
    def total_calls_preflop(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Pre-Flop", {}).get("Call", 0)
            for role in self.action_counts
        )
    ...
    def total_folds_postflop(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get(stage, {}).get("Fold", 0)
            for role in self.action_counts
            for stage in post_flop_stages
            if stage in self.action_counts.get(role, {})
        )
    ...
    def total_calls_postflop(self):
        ...
        post_flop_stages = ["Flop", "Turn", "River"]
        return sum(
            self.action_counts.get(role, {}).get(stage, {}).get("Call", 0)
            for role in self.action_counts
            for stage in post_flop_stages
            if stage in self.action_counts.get(role, {})
        )
    ...
    def total_folds_flop(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Flop", {}).get("Fold", 0)
            for role in self.action_counts
        )
    ...
    def total_bets_flop(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Flop", {}).get("Bet", 0)
            for role in self.action_counts
        )
    ...
    def total_raises_flop(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Flop", {}).get("Raise", 0)
            for role in self.action_counts
        )
    ...
    def total_calls_flop(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Flop", {}).get("Call", 0)
            for role in self.action_counts
        )
    ...
    def total_checks_flop(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Flop", {}).get("Check", 0)
            for role in self.action_counts
        )
    ...
    def total_folds_turn(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Turn", {}).get("Fold", 0)
            for role in self.action_counts
        )
    ...
    def total_bets_turn(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Turn", {}).get("Bet", 0)
            for role in self.action_counts
        )
    ...
    def total_raises_turn(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Turn", {}).get("Raise", 0)
            for role in self.action_counts
        )
    ...
    def total_calls_turn(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Turn", {}).get("Call", 0)
            for role in self.action_counts
        )
    ...
    def total_checks_turn(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("Turn", {}).get("Check", 0)
            for role in self.action_counts
        )
    ...
    def total_folds_river(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("River", {}).get("Fold", 0)
            for role in self.action_counts
        )
    ...
    def total_bets_river(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("River", {}).get("Bet", 0)
            for role in self.action_counts
        )
    ...
    def total_raises_river(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("River", {}).get("Raise", 0)
            for role in self.action_counts
        )
    ...
    def total_calls_river(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("River", {}).get("Call", 0)
            for role in self.action_counts
        )
    ...
    def total_checks_river(self):
        ...
        return sum(
            self.action_counts.get(role, {}).get("River", {}).get("Check", 0)
            for role in self.action_counts
        )
    ...
    def calculate_vpip(self):
        return (
            (self.total_vpip_hands / self.total_hands) * 100
    ...
    def calculate_pfr(self):
        return (
            (self.total_pfr_hands / self.total_hands) * 100
            if self.total_hands > 0
            else 0
        )
    ...
    def calculate_cbet_frequency(self):
        return (
            (self.total_cbets_made / self.total_cbet_opportunities) * 100
            if self.total_cbet_opportunities > 0
            else 0
        )

    def calculate_wtsd(self):
        return (
            (self.total_hands_went_to_showdown / self.total_hands) * 100
            if self.total_hands > 0
            else 0
        )
    ...
    def calculate_3bet_frequency(self):
        return (
            (self.total_3bets / self.total_3bet_opportunities) * 100
            if self.total_3bet_opportunities > 0
            else 0
        )

    def calculate_fold_to_3bet_frequency(self):
        return (
            (self.total_folds_to_3bet / self.total_3bet_opportunities) * 100
            if self.total_3bet_opportunities > 0
            else 0
        )
    ...
    def calculate_steal_attempt_frequency(self):
        return (
            (self.total_steal_attempts / self.total_steal_opportunities) * 100
            if self.total_steal_opportunities > 0
            else 0
        )
